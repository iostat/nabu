...
public abstract class Component {
    ...
    /**
     * (internal) validates that this component is in a valid state to stop and dispatches
    ...
    void _dispatchShutdown() throws ComponentException {
        if(wasStopped()) { logger.warn("Trying to stop an already stopped {}", this.getClass().getCanonicalName()); return; }
        if(!wasStarted()) {
            logger.warn("Trying to stop a non-started {}", this.getClass().getCanonicalName());
            return;
        }

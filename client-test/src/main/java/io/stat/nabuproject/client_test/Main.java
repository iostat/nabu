...
public class Main {
    ...
    private void start() throws Throwable {
        ...
        IndexTest it = new IndexTest(client, oks, retrs, qs, fails, submitExcs, futureExcs, parseTime, performTime, totalDocs);

        // 100 threads max for our parallel loader stream...
        // 1500 thread writer pool ;)
        ForkJoinPool fjp = new ForkJoinPool(FJP_SIZE);
        NamedThreadPoolExecutor executor = new NamedThreadPoolExecutor("Perform Pool", TP_SIZE, TP_SIZE, 1, TimeUnit.MINUTES, new SynchronousQueue<>(true));
